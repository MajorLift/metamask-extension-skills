...
Team-specific, battle-tested engineering knowledge packaged as skills that AI coding agents ([Claude Code](https://claude.com/claude-code), [Cursor](https://cursor.com), [OpenCode](https://opencode.ai)) load on demand.

## Quickstart

### Prerequisites

- Node ≥ 22.6 (TS type stripping is built-in from 22.6+; default from 23.6; stable in 24)
- [`gh` CLI](https://cli.github.com/) authenticated: `gh auth login`
- Write access to this repo (ask for a collaborator invite)
- `git` on your PATH (used for `--source-repo` auto-detect)

No npm deps. `tools/remember.ts` shells out to `gh`.

### Capture a skill from the command line

```bash
node tools/remember.ts \
  --domain pr-workflow \
  --body-file /tmp/shaped-body.md \
  "Prefer squash-merge for feature branches under 10 commits."
```

`--body-file` is markdown with section headings (`## When to use`, `## Do not use when`, `## Notes`) below an implicit H1 title. Omit it only if you're happy with TODO placeholders (you usually aren't — the tool will emit a follow-up nudge).

### Capture from a PR

On any open PR in `MetaMask/metamask-extension`:

```
@metamaskbot /remember <one-liner>
```

Direct-commit tier writes immediately; PR tier opens a PR on this repo; reject tier ignores drive-bys. See the [`remember` meta-skill](domains/meta/skills/remember/skill.md) for the full contract.

### Amend an existing skill

```bash
node tools/remember.ts \
  --edit domains/ui-performance/skills/selector-anti-pattern-review/skill.md \
  --body-file /tmp/amendment-body.md \
  "Add caveat: reselect memoization breaks under prop-drilled non-primitive args."
```

Appends an `## Amendment <timestamp>` section plus a changelog entry. No overwrite, no force-push.

### Consume the bundle

Agents pull synced copies into `.cursor/rules/`, `.agents/skills/`, or `.claude/skills/` via `yarn skills:sync` in the consumer repo. The sync is pinned to a SHA recorded in `.skills/VERSION`.
...
Every meta-skill invocation returns two blocks: an **Outcome** (paths + SHAs + URLs) and a conservative **Follow-ups** list (concrete action items only when warranted; empty by default). Agents consuming the output can trust silence.

### [`remember`](domains/meta/skills/remember/skill.md)

The capture surface. `/remember` folds four responsibilities into one invocation:
...
3. **Pick a domain** — match against existing domains and route the capture directly there. There is no inbox staging area with a curator; skills stuck in staging never ship.
4. **Decide new-vs-edit** — search the bundle for overlap first; amend an existing skill when one already covers the ground, create a new entry only when none does.
...
Three skills that illustrate what passes the admission filter and what the method looks like when it works.
...
Examples here are drawn from MetaMask browser extension and mobile work, but the method generalizes to any team running agents in the loop.
